...
const ACTOR_STYLES: Record<string, { label: string; color: string; icon: typeof User }> = {
  "مجری": { label: "مجری", color: "bg-blue-500/10 border-blue-500/20 text-blue-400", icon: User },
  "مجری / مالک": { label: "مجری / مالک", color: "bg-blue-500/10 border-blue-500/20 text-blue-400", icon: User },
  "کارشناس": { label: "کارشناس", color: "bg-amber-500/10 border-amber-500/20 text-amber-400", icon: Briefcase },
  "مدیر واحد نظارت گازرسانی": { label: "مدیر واحد", color: "bg-emerald-500/10 border-emerald-500/20 text-emerald-400", icon: Shield },
  "سامانه": { label: "سامانه", color: "bg-slate-500/10 border-slate-500/20 text-slate-400", icon: Monitor },
};
...
function StepCard({
  ...
  const isDecision = step.isDecision || hasBranches;
  const actorStyle = step.actor ? (ACTOR_STYLES[step.actor] ?? ACTOR_STYLES["مجری"]) : null;
  const ActorIcon = actorStyle?.icon ?? User;
  ...
  return (
    <div ref={ref} className="relative flex gap-6 pb-0">
      ...
      >
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                {/* Actor badge */}
                {actorStyle && (
                  <span className={`inline-flex items-center gap-1 text-xs font-medium border px-2.5 py-0.5 rounded-full ${actorStyle.color}`}>
                ...
                {/* Timer badge */}
                {step.timer && (
                  <span className="inline-flex items-center gap-1 text-xs font-medium text-cyan-400 bg-cyan-500/10 border border-cyan-500/20 px-2.5 py-0.5 rounded-full">
                    <Clock className="w-3 h-3" />
                    {step.timer}
                  </span>
                )}
                {isDecision && !hasBranches && (
          ...
          <div className="px-6 pb-4">
            ...
            <div className="space-y-2">
              {step.branches!.map((b, bi) => (
                <div key={bi} className={`flex items-start gap-2 px-3 py-2.5 rounded-xl border ${BRANCH_COLORS[b.color]}`}>
                  <ChevronLeft className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 rotate-180" />
                  <div>
        ...
        {/* Legacy decision (yes/no) */}
        {step.isDecision && step.decision && !hasBranches && (
          <div className="px-6 pb-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {step.requiredDocs.map((doc, di) => (
                    <span key={di} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs bg-blue-500/10 border border-blue-500/20 text-blue-400">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {step.tools.map((tool, ti) => (
                    <span key={ti} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs bg-violet-500/10 border border-violet-500/20 text-violet-400">
